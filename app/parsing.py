...
for index, row in data.iterrows():
    ...
    if passage_embedding is not None:
        passage_embedding = passage_embedding[0]

        # Append the passage and metadata to their respective lists
        passages.append(passage)
        metadata.append(row['Metadata'])

        # Append the embedding as a single value to the embeddings list
...
if embeddings:
    ...
    output_data = pd.DataFrame({'Passage': passages, 'Metadata': metadata})

    # Add the embeddings as separate columns in the DataFrame
    for i in range(embeddings.shape[1]):
        output_data[f'Embedding_{i}'] = embeddings[:, i]
